...
// Split on shell logical operators only. Pipes stay inside a clause so the
// destructive scan still catches things like `find . | xargs rm`.
...
export function isSafeCommand(command: string): boolean {
	...
	for (const clause of clauses) {
		...
		if (!SAFE_PATTERNS.some((p) => p.test(clause))) return false;
	}
	return true;
}
