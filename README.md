...
- **Multi-provider models**: z.ai (GLM-5.1), OpenAI (GPT-5.3 Codex), OpenRouter (Claude, Kimi K2, Gemini). All resolved from env vars at runtime.
- **Extensions**: status bar, `/cost`, `/effort`, `/clear`, questionnaire tool.
- **Skills**: commit (atomic, style-mimicking), docs (codebase documentation updater), cleanup (AI artifact removal).
...
export BRAVE_API_KEY=...         # Brave Search (web tools)
```
...
pi() {
    (
        ...
        set +a
        exec srt --settings "$HOME/.pi/agent/srt.json" pi "$@"
    )
}
...
Configured in `config/models.json`. Three providers:
